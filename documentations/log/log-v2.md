...
| D21 | Where does the builder export the nail-outline curve D20's sampler needs: in step 3 now, or in step 5? | **Step 5**, together with the sampler change that is its only consumer. Left to the main session by the user ("自行决定，采效率最高&&最优解，均衡二者" — decide for the most efficient and best result, balancing both): it saves a step-3 fix round and keeps the interface change next to its consumer. |
| D22 | Step-3 digits fix: the long thumb-root cap (item 5) removed the proud capsule end and its groove but leaves a soft fold line along the right thumb's underside (home view x 1127–1194, y 725–727; a slit in the −35° view), because the thumb's base hangs over palm 0.05–0.17 world units behind it (the pose puts the CMC at z 0.24). Keep it, add a thenar filler now, or go back to the short cap? | **Keep it as a thenar crease** (the fixer's option A), decided by the main session under the same delegation. Step 2 shortens it by moving the right thumb CMC back in depth where the reference allows (z 0.24 → 0.20 / 0.16 shortens it from 67 px to 43 / 20 px). Rejected: a thenar filler would change the thumb base in every view and cost another builder-and-verify cycle for a minor mark; the short cap would undo item 5. |
| D23 | Step 2 (run 3), left hand: D18 asks the curled middle and ring fingers for 3D P1/P2 1.4-1.6 with their PIPs moved away from the camera, but with hinged fingers (PIP/DIP bends in the MCP flexion plane, small MCP abduction) the drawn knuckle row turns the back of the hand about 17 deg away from the camera, palmar flexion then brings the PIPs toward it, and inside the middle MCP's joint gate middle P1/P2 reaches only 1.21 (ring 1.36). The committed R7 met D18's numbers (1.45 / 1.53) only by bending the middle and ring fingers -64 / -49 deg sideways at the MCP with IP bends up to 23 deg out of the hinge plane. Which pose goes into the repo: (A) hinged fingers, (B) keep R7, (C) re-read the middle MCP keypoint? | **(A) hinged fingers**, chosen by the user (2026-09-24). Candidate A passes every left gate (IoU 0.983, contour mean 0.79 / p95 2.0 px, negative space 0.932; MCP abduction <= 6 deg, IP bends <= 0.3 deg out of the hinge plane, knuckle line 11 deg off). D18 is amended: palmar flexion in the hinge plane, with P1/P2 what the drawing allows. Rejected: (B) the sideways MCP bends are the kinked-joint defect the right verifier rated major, and the fingers hook sideways seen from above; (C) needs a change to the reference keypoints and another calibration run. Evidence: `outputs/qa/calib/compare-left/decision-views-R7-vs-candA.png`, report `outputs/qa/calib/reports/step2-run3-left-calib-paused.json`. |
| D24 | Only with D23 (A): the turned back of the hand rolls the left thumb's frame with it, so at thumb_roll_deg 72 (D20) the thumbnail turns 25.7 deg off the camera and reads partly as a band. Change thumb_roll_deg to 95? | **Yes, 95**, chosen by the user (2026-09-24): an exception to D20 for the left thumb only. The nail faces the viewer 2.3 deg off the camera, as in the reference and as D2 reads the thumb; its shape, relief and outline are unchanged (every gate passes: IoU 0.9833, p95 2.0). Rejected: 72, which keeps the parameter but not the nail's orientation. Evidence: `outputs/qa/calib/compare-left/decision-thumbnail-5x.png`. |
| D25 | verify:left#1 (step 2 run 3) passed every left gate but rated a major: the left thumb is not straight in 3D. Its metacarpal lies along the hand axis (thumb_cmc pushed 2.86u toward the wrist to lay the thenar on the palm-heel outline) and it turns 24.8 deg sideways at the MCP, with the IP 7.9 deg the other way; the drawn thumb is straight, coming diagonally out of the base of the palm (D2). Accept it (A), refit it with a straightness constraint (B), or change the builder first (C)? | **(B) one bounded refit**, chosen by the user (2026-09-24): MCP sideways <= about 12 deg, IP sideways <= about 5 deg, thumb_cmc toward its reading (430,330), thenar, palm heel and wrist crease refitted to the palm-heel outline; the thumb tip moves about 9 px back along IP->tip (the verifier's e7, every gate still passes). If p95 then stays above 2.0, the committed thumb stays with the residual recorded (D6), e7 still applies, and (C) is recorded as a builder request for a later round (thenar and palm heel independent of the thumb metacarpal, thumb frame from the nail). Evidence: `outputs/qa/calib/compare-left/decision-thumb-axis-3x.png`, `decision-groove-committed-e5-e1.png`; verdict `outputs/qa/calib/reports/step2-run3-left-verify1.json`. |
...
| D10 (answer) | Step 5 measured the particle cloud with the reference right mask's own density rule (medium tier, 5 seeds: IoU 0.868–0.887, contour mean 5.1–6.8 px, p95 12–28 px, negative space 0.68–0.74 against the right mesh gates 0.918 / 3.7 / 8.9 / 0.825). Which gate? (A) the right mesh gates, with a sampler redesign; (B) a regression gate at the measured level; (C) against the app's own mesh silhouette; (D) none. | **(B)**, chosen by the user (2026-09-25): `scripts/particle_shape.py` at the default tier, IoU ≥ 0.85, contour mean ≤ 7.5 px, negative space ≥ 0.65, index tip ≤ 8 px, other tips ≤ 9 px; p95 reported, not gated; the low tier recorded, not gated. Whether the particles read like the drawing is judged in step 6. |
| D26 | How to run step 6 (the adversarial review against review §E's five questions): a multi-agent workflow (one reviewer per question, each finding verified independently; ~1.5–3 h, ~1–2M tokens) or a single reviewer agent? | **Multi-agent workflow**, chosen by the user (2026-09-25), run attended in the main session. |
