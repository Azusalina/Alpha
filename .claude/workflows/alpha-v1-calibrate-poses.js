...
Read first:
- documentations/log/log-v2.md: decisions D1-D22, "Step 2 — calibration runs" and "Resume here" (this run is step 2 again, on the builder that step 3 widened).
...
- Step 3's notes for this hand: the palm is now anatomical in width but still 40-44 mm thick on its ulnar half against about 30 mm (palm flat, hypothenar_* and fdi_* set it; the arm builder's demo pose dR1 in step3-arm-final.json passes every right gate with only those controls); the 6.3 px bow in the index's top line is the pose's 13.8 deg PIP bend (index_pip 473 -> 479 px, 0.9 u, gave top-line rms 3.07 -> 1.85 px); thumb_cmc is 45.5 px off its reading (gate 75); without the old collar ring the wrist's top edge (x 1280-1330) sits about 2 px farther from the drawn outline.`
...
- Step 3's notes for this hand: on the committed pose the new builder scores IoU 0.958, contour mean 1.88 / p95 5.0 px (index tip 3.16 px against 3); the pose still uses the old knuckle shape (knuckle_rise 0, head_back 0, phalanx_base 1.0), so set those and move the index, middle and ring MCPs back to their readings (they sit about 26 px off); the arm builder's demo poses dL1-dL4 in step3-arm-final.json show the forearm sag, wrist bump and palm heel at work (dL4: IoU 0.965, mean 1.59, p95 4.0); the default hypothenar (hypothenar_from 0.05) is part of the wrist-crease residual; the knuckle line is 51 deg off its lateral axis toward the palm (the right's is 3.9 deg), which tilts the metacarpal plate in 3D (a dorsal item).
