...
- D12 (2026-09-21) The RIGHT reference mask BRIDGES the three dorsal bays along the back of the index finger and the knuckles (x 867-1008, y 441-514): the back of the finger runs straight, as the particles and their joining lines do. Bridging is the default in scripts/reference_masks.py, and the gates are derived from the bridged mask.
- D13 (2026-09-21) The LEFT thumb tip keypoint is the measured (562, 458) with uncertainty 8 px (the distal end of the thumb along its axis); D2's approximate reading (548-559, 461) only named the digit. No gate depends on this point.
- D14 (2026-09-21) The left gates keep the +-1 px dilate/erode stroke variant, although the corrected stroke half-width reaches 1.07 px at p90 (median 0.82 px); the caveat stays documented in ACCEPTANCE.md section 4.1.
(The full decision list, D1-D15, is in documentations/log/log-v2.md.)
...
- Decisions D12-D14 are applied: right-mask.png bridges the dorsal bays (the back of the index finger and the knuckles runs straight across x 867-1008, y 441-514 — look at a zoomed crop) and thresholds.json is derived from the bridged mask; outputs/qa/reference/bays-bridged/ is gone; keypoints.json left.thumb_tip is (562, 458) with uncertainty 8 px; meta.json and ACCEPTANCE.md record D12, D13 and D14 as user decisions, not as open questions.
...
const RESUME_SCOPE = {
  reference: 'STATE AT THE START OF THIS RUN (2026-09-21): D5 is applied and committed, and two verify/fix rounds ran. They fixed a sign error in stroke_width_fwhm() (the stroke is median 1.64 px wide, not 1.44) and several ACCEPTANCE.md wording issues, and they correctly left three questions to the user, who has now decided them: D12 (bridge the right dorsal bays), D13 (keep the left thumb tip at the measured (562, 458), uncertainty 8 px) and D14 (keep the +-1 px left stroke variant). REMAINING IN THIS RUN: (a) apply D12: make bridging the default in scripts/reference_masks.py (the --bridge-bays path already exists; keep the unbridged variant reachable only if that is cheap and clearly labelled as not the reference), regenerate with python3 scripts/reference_masks.py --sensitivity, and record D12 in meta.json as a user decision (no longer an open question); (b) record D13 and D14 the same way: meta.json left.known_ambiguities and the ACCEPTANCE.md paragraph that says the thumb tip is not settled become user decisions; (c) update docs/ACCEPTANCE.md everywhere the right-hand numbers or the D12/D13/D14 status appear (commands table, section 2, the section 4 gate tables and the looser-than-the-log reasoning, 4.3, section 5), and list the final gate values in your metrics; (d) delete outputs/qa/reference/bays-bridged/ once bridging is the default (it was decision-support output only); (e) re-run your own checks (byte-identical fresh run, both self-tests) and change nothing else that is correct. Leave ACCEPTANCE.md section 6 (Blender baseline) and outputs/qa/reference/selftest/blender-baseline-silhouette/ to the main session, which re-measures them after this run.',
  blender: 'STATE AT THE START OF THIS RUN (read it before doing anything): an earlier builder run made most of these fixes and was then cut off by the account session limit without reporting. Its work is committed: build_hands.py (contour extraction reworked, a printed [d9] self-check, surface fixes), hands.blend, both GLBs, both contour JSONs, the masks, the four views per hand, the mesh reports, and a new assets-source/hands/check_contours.py. The main session then re-ran the documented rebuild and checked: GLBs, masks and views are byte/pixel-identical across rebuilds, the contour JSONs are identical across rebuilds (they had been stale on disk before that rebuild), the A1 numbers hold (1 shell, 0 non-manifold, 0 boundary, 100 % winding, 28 854 triangles), and D9 coverage measured independently is 100 % with a largest uncovered run of 0 px on BOTH hands. The fold on the back of the left hand and the nail-edge defects are gone from the shaded views. SO WHAT IS LEFT IS SMALL: (a) docs/HAND_ASSETS.md was never updated and is now wrong in several places (the fold called invisible at an oblique angle, "two" rebuilds, the contour section and its coverage claim, any PARAMS you changed, the current-state tables, the determinism claim); bring it in line with what the code and the outputs actually do now; (b) one short kind=="inner" polyline on the LEFT hand (6 points) lies about 1.7 px from the mask boundary for 4 of its 6 points, although the builder-side "inner on background" check reports 0 — decide whether that is a legitimate tangency or a mislabel, say which and why, and fix it if it is a mislabel; (c) verify the state yourself rather than trusting this note, and return a BUILD_RESULT describing the FINAL state. Do not redo what is already correct, and do not change poses or radii. For the record, the original findings were: ' +
    'fix the open findings of the last independent verifier (documentations/log/log-v2.md, "Blender verifier findings"): (1) MAJOR: the kind=="outer" polylines in hand-<hand>.contour.json must cover the mask boundary to the pass criterion of decision D9 (at session start: left 94.3 % with a 36 px gap at the wrist underside, right 98.3 % / 10 px; some real outline is labelled inner); report coverage % and the largest uncovered run for both hands, measured as D9 defines them; (2) the decimation fold on the back of the left hand (~482,251 px), visible as a bright slash in the home and +-35 deg views, and correct HAND_ASSETS.md, which calls it invisible; (3) nail-edge surface defects (left thumb nail creases, right thumb underside notch, right pinky tip dent, left index nail dimple); (4) HAND_ASSETS.md says two rebuilds where there were three. The left reference mask and the gates are being regenerated in parallel in this run (decision D5), so any compare_silhouette numbers you get are provisional: report them as such and do not iterate on them; the main session re-runs the comparison after step 1. Do NOT change poses or radii: pose-right depth reuse and form calibration are step 2 (D3); the forearm cuff is step 3. Keep every A1 number (1 shell, 0 non-manifold, 0 boundary, winding > 99.5 %, <= 30k triangles).',
