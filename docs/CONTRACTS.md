...
| `public/assets/hand-<hand>.contour.json` | `{"polylines": [[[x, y, z], …], …], "camera": {…}}` — silhouette edges from the home camera, app world, longest polyline first. Source for the drawn outer contour. |
...
(mean / p95 / max px), negative-space IoU, keypoint offsets. Overlay colours
(review A4): **reference only = red, render only = blue, overlap = black**, on
white. Refuses any input that is not 1644 × 957.

`scripts/overlay_check.py` (app screenshots) — **not yet rewritten**; see the
log. Target behaviour: take a silhouette-mode screenshot (§9), split hands by ID
colour, run the comparison above for both; also accept a full-render screenshot
for the two-ink overlay only.
